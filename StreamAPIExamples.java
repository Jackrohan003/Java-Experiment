...
//		filterExamples();
	...
	private static void mapExamples() {
		...
		List<Integer> multiplyByTwo = ints.stream().map(e -> e * 2).collect(Collectors.toList());
		...
		// Everywhere else where you have to pass a function or a predicate, you can use
	...
	private static void filterExamples() {
		...
		System.out.println("Find the Word that is not Palindrome");
		List<String> notPalindromes = words.stream().filter(s -> !s.equals(new StringBuilder(s).reverse().toString()))
				.collect(Collectors.toList());
	...
	private static void moreFunnyExamples() {
		...
		String Delim = words.stream().collect(Collectors.joining(" "));
		System.out.println(Delim);
		
		String withDelimiter = words.stream().map(it -> Integer.toString(it.length()))
				.collect(Collectors.joining(" + "));
		System.out.println(withDelimiter);

		System.out.println("2. Concat two streams:");
		List<String> concat = Stream
				.concat(words.stream(), words.stream().map(s -> new StringBuilder(s).reverse().toString()))
				.collect(Collectors.toList());
		System.out.println(concat);

		System.out.println("3. Create a Map from stream");
		// toMap takes two arguments: the first is the function to get keys (we pass
		// identity there -- it is a function
		// that returns exactly its argument, like `x -> x`, and the second one to get
		// values.
		...
		// This example is a bit complex, but it can be useful to learn the concept of
		// `reducing`.
		// `reduce` is an operation with an accumulator: we provide an initial value for
		// the accumulator and a function
		// with two arguments. As one of the arguments the current accumulator value
		// will always be passed.
		// The function is applied to every item in the stream and its result is the
		// next value of the accumulator.
		//
		// Now let's try to implement ints production which, unfortunately, is not there
		// along with `sum` and `average`.
		int production = Stream.of(1, 2, 3, 5, 7, 11).reduce(1, (acc, i) -> acc * i);
		System.out.println(production);

		System.out.println("8. Int range");
		// Yeah, `mapToObj(i -> i)` looks weird, but in Java IntStream and Stream are
		// not the same, and this is the
		...
		// Flat map is similar to map: it iterates over every item in the stream and
		// calls the function which you
		// pass as the argument. But your function should return not a single value but
		// a stream. And flat map then
		// concatenates all the streams returned.
		List<Character> chars = words.stream().flatMap(w -> w.chars().mapToObj(c -> (char) c))
				.collect(Collectors.toList());
		System.out.println(chars);

		System.out.println("10. Group by");
		// The result is a map where for each value of the function passed to groupingBy
		// there is a list of the stream
